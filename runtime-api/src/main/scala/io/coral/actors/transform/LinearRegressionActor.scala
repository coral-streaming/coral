...
//  }
  ...
  override def trigger = {
    json =>
      val entries = weights.toList
      val inputVector = entries.map(e => {
        (json \ e._1).extractOpt[Double] match {
          case Some(value) => value
          case None => throw new Exception("Key does not exists")
        }}).toVector

      result = intercept + (inputVector zip entries.map(_._2)).map(x => x._1 * x._2).sum
      Done
  }
